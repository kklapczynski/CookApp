...
// dispatch login action - replace login with service in a
export class AuthEffects {
    
    constructor(private actions$: Actions, private http: HttpClient, private router: Router) {}   // ther is convention to mark observables with '$', but it's not required
        ...
        switchMap((authData: AuthActions.LoginStart) => { // switchMap creates Observable by taking another observable's data
            return this.http.post<AuthResponseData>(
                ...
                map(resData => {
                    ...
                    return new AuthActions.Login({ // pass an action: @Effect decorator makes ngrx dispatch action passed here auctomatically
                        email: resData.email,
                ...
                catchError(errorRes => {
                    ...
                    return of(new AuthActions.LoginFail(errorMessage))    // creates new observable without error, inside we should pass an action @Effect decorator makes ngrx dispatch action passed here auctomatically
                })
